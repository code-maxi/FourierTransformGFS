...
def get_svg_points(name: str, precision: int, size: float):
    ...
    return np.array([(p - (minx + maxx)/2 - (miny + maxy)/2 * 1j) * scale for p in points])

def find_coefficients(points: list[complex], N: int):
    return np.array([sum([z * np.exp(-2j*np.pi * n * x / len(points)) / len(points) for x, z in enumerate(points)]) for n in range(-N, N+1)])
